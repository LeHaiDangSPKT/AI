...
if __name__ == "__main__":
    ...
    if train_mode:
        ...
        for iter_no, batch in enumerate(get_batches(env, net, BATCH_SIZE)):
            ...
            if reward_m > 200:
                torch.save(net, 'trained_net_' + str(HIDDEN_SIZE) + 'neurons.pt')
                print("Solved! Saved the model")
                break
    else:
        env = gym.wrappers.Monitor(env, directory="solve_cartpole", force=True)
        obs = env.reset()
        net = torch.load('trained_net_10neurons.pt')

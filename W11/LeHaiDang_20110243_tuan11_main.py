...
def min_conflicts(csp, max_steps=100000):
    ...
    #max-steps: số lần lặp tối đa trước khi trả về kết quả Failure (None)
     
    csp.current = current = {} # Tạo một current là một dictionary rỗng
    
    #Gán mỗi var của tập variable một giá trị (chính là giá trị số cặp conflicts nhỏ nhất, nếu có nhiều giá trị min thì chọn random từ các vị trí min)
    for var in csp.variables:
        val = min_conflicts_value(csp, var, current)
        '''Thêm vào dictionary một cặp {key:value} tương ứng là {var:val} thay thế cho giá trị val cũ nếu tồn tại key là var trong dictionary''' 
        csp.assign(var, val, current)


    for i in range(max_steps):
        #Xác định xem có conflicted nào hay không
        conflicted = csp.conflicted_vars(current)

        #Nếu không có conflicted thì trả về một solution
        if not conflicted:
            return current
        
        #Chọn ngẫu nhiên một biến bị conflict trong tập Variable vẫn còn trong phạm vi ràng buộc
        var = random.choice(conflicted) 

        #Tính giá trị value cho biến var tương ứng là giá trị conflicts nhỏ nhất hoặc random 
        val = min_conflicts_value(csp, var, current)

        #Gán giá trị var = val cho trại thái current
        csp.assign(var, val, current) #Thêm giá trị {var:val} vào dictionary current, thay thế giá trị val cũ nếu đã tồn tại var

    #Sau khi thực hiện lặp với số lần max_steps mà vẫn chưa tìm ra solution thì return None    
    return None
